...
<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        //.wrap("<div class='dataTables_borderWrap' />")   //if you are applying horizontal scrolling (sScrollX)
            .DataTable({
                ...
                "columns": [
                    ...
                    {"data": "expression", "sClass": "center"},
                    {"data": "charset", "sClass": "center"},//4
                    {"data": "settingTime", "sClass": "center"},
                    {"data": "regularID", "sClass": "center"}
                ],
                ...
                'columnDefs': [
                    ...
                    {
                        "orderable": false, className: 'text-center', "targets": 2, title: '段落正规式', render: function (data, type, row, meta) {
                            return HtmlUtil.htmlEncode(data);
                        }
                    },
                    {
                        "orderable": false, className: 'text-center', "targets": 3, title: '提取正规式', width: 200, render: function (data, type, row, meta) {
                            var text = "";
                            jQuery.each(data, function (key, val) {
                                text += val["expressionName"] + ", ";
                            });
                            return text.substring(0, text.length - 2);
                        }
                    },
                    {"orderable": false, className: 'text-center', "targets": 4, title: '文件字符集', width: 110},
                    {"orderable": false, 'targets': 5, 'searchable': false, title: '设置时间', width: 130},
                    {
                        "orderable": false, 'searchable': false, 'targets': 6, title: '操作', width: 80,
        ...
        function showRegularDialog(regular) {
                ...
                'columnDefs': [
                    {
                        "orderable": false, "searchable": false, className: 'text-center', "targets": 0, width: 20, render: function (data, type, row, meta) {
                            for (var i = 0; i < regular.expression.length; i++) {
                                if (data === regular.expression[i].expressionID) {
                                    return '<input type="checkbox" data-id="{0}" checked>'.format(data);
                                }
                            }
                            return '<input type="checkbox" data-id="{0}">'.format(data);
                        }
                    },
            ...
            expressionTable.on('draw', function (e, settings) {
                //加载时，通过render函数增加了checked，这里把整行选上
                $('#expression-table tr').find('input[type="checkbox"]:checked').parent().parent().each(function (index, element) {
                    expressionTable.row(element).select();
                });
            });
...
<div id="dialog-edit" class="hide" style="z-index: 10">
    <div class="col-xs-12" style="padding-top: 10px">
        ...
        <form class="form-horizontal" role="form" id="regularForm">
            ...
            <div class="row">
                <label for="expression-table">选择需要提取的内容：</label>
